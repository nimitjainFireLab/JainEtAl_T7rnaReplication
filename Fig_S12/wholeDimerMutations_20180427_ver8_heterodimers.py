            ...
            if isFullLength(newObj):               
                if returnNumberMutations(newObj.alignmentInfo[1],boundsFirstHalf)<=numberMutationMax and returnNumberMutations(newObj.alignmentInfo[1],boundsSecondHalf)<=numberMutationMax:
                    ...
                    if len(firstDiv)==diversityPosNumber and len(secondDiv)==diversityPosNumber:
                        ...
                        if firstDiv!=secondDiv and numberDiffHalves(firstDiv,secondDiv)>1: #the numberDiffHalves cutoff is for getting rid of potential polymerase error cases
...
plt.savefig(sampleName+'_wholeDimerMutations_20180427_ver8_heterodimers.pdf',dpi=300)
with open(sampleName+'_wholeDimerMutations_20180427_ver8_heterodimers.pckl','wb') as f:
