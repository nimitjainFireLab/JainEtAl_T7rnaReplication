...
def medianLengthTwoWayRepeat(coordVector,sequence):
    return np.median([len(sequence[coordVector[0]:coordVector[2]+1]),len(sequence[coordVector[3]:coordVector[1]+1])])

def medianLengthFourWayRepeat(coordVector,sequence):
    if coordVector==[]:
        return 0.0
    else:
        return np.median([len(sequence[coordVector[0]:coordVector[1]+1]),len(sequence[coordVector[2]:coordVector[3]+1]),len(sequence[coordVector[4]:coordVector[5]+1]),len(sequence[coordVector[6]:coordVector[7]+1])])

def percentageLengthTwoWayRepeat(coordVector,sequence):
    return (float(len(sequence[coordVector[0]:coordVector[2]+1])+len(sequence[coordVector[3]:coordVector[1]+1]))/float(len(sequence)))*100.0

def percentageLengthFourWayRepeat(coordVector,sequence):
    if coordVector==[]:
        return 0.0
    else:        
        return (float(len(sequence[coordVector[0]:coordVector[1]+1])+len(sequence[coordVector[2]:coordVector[3]+1])+len(sequence[coordVector[4]:coordVector[5]+1])+len(sequence[coordVector[6]:coordVector[7]+1]))/float(len(sequence)))*100.0
    
colorPaletteDictionary={
        '1.1': (0,88,0),
        '2.1':  (246,132,9),
        '2.2': (211,230,0),
        '3.1': (149,211,79),
...
for counter, eachSeq in enumerate(actualSeq):
    print namesSeq[counter]
    print eachSeq
    twoWayResults=extendTwoWayRepeat(returnElementNotation(eachSeq),eachSeq)
    twoWayRepeatCoord.append(twoWayResults[1:])
    resultantStrings=multiAlWrite(twoWayResults[0],eachSeq,twoWayResults[1],twoWayResults[2],twoWayResults[3],twoWayResults[4])            
    newDotBracket=removeGUWobble(resultantStrings[0],resultantStrings[1])
    print newDotBracket
    fourWayRepeatCoord.append(returnFourWayRepeatIndices(newDotBracket,resultantStrings[0],resultantStrings[1],resultantStrings[2],resultantStrings[3],eachSeq))
    printTwoWayRepeat(twoWayRepeatCoord[counter],eachSeq)
    printFourWayRepeat(fourWayRepeatCoord[counter],eachSeq)
    lengthSequence.append(len(eachSeq))
    lengthStem.append(medianLengthTwoWayRepeat(twoWayRepeatCoord[counter],eachSeq))
    lengthFourWay.append(medianLengthFourWayRepeat(fourWayRepeatCoord[counter],eachSeq))
    normalizedStem.append(float(lengthStem[counter])/float(len(eachSeq)))
    normalizedFourWay.append(float(lengthFourWay[counter])/float(len(eachSeq)))
    lengthRNAInTwoWay.append(percentageLengthTwoWayRepeat(twoWayRepeatCoord[counter],eachSeq))
    lengthRNAInFourWay.append(percentageLengthFourWayRepeat(fourWayRepeatCoord[counter],eachSeq))
    print lengthStem[counter]
    print lengthFourWay[counter]
    print normalizedStem[counter]
    print normalizedFourWay[counter]
    print lengthRNAInTwoWay[counter]
    print lengthRNAInFourWay[counter]

f, axes=plt.subplots(5,1)
f.set_size_inches((2,10))
for counter, eachArray in enumerate(abundanceSeq):
    axes[0].scatter(eachArray[0],eachArray[1],c=(np.array(colorPaletteDictionary[namesSeq[counter]])/255.0).reshape((1,3)),edgecolors='none',s=15)

minLimit=-10
maxLimit=80
axes[0].plot(np.arange(minLimit,maxLimit),np.arange(minLimit,maxLimit),'k--',linewidth=0.25)
axes[0].plot(np.arange(minLimit,maxLimit),2*np.arange(minLimit,maxLimit),'k--',linewidth=0.25)
axes[0].plot(np.arange(minLimit,maxLimit),0.5*np.arange(minLimit,maxLimit),'k--',linewidth=0.25)
axes[0].set_xlim([minLimit,maxLimit])
axes[0].set_ylim([minLimit,maxLimit])
axes[0].set_xlabel('Sequence abundance [%]')
axes[0].set_ylabel('Rev. comp. abundance [%]')
axes[0].set_aspect('equal')

lengthAlreadySeen={}
baseValueY=1
yLinearGradient=0.2
multiplicativeFactor={True:1, False:-1}
for counter, eachEl in enumerate(lengthSequence):
    xToPlot=eachEl    
    if eachEl in lengthAlreadySeen:
        yToPlot=baseValueY+multiplicativeFactor[lengthAlreadySeen[eachEl]%2==0]*yLinearGradient*((lengthAlreadySeen[eachEl]/2)+1)
        lengthAlreadySeen[eachEl]+=1
    else:
        yToPlot=baseValueY
        lengthAlreadySeen[eachEl]=0
    axes[1].scatter(xToPlot, yToPlot,c=(np.array(colorPaletteDictionary[namesSeq[counter]])/255.0).reshape((1,3)),edgecolors='none',s=15)
axes[1] .set_xlim([60,110])
axes[1].set_ylim([0,2])
axes[1].set_xlabel('Length of sequence [bases]')

boxPlotData=[np.array(normalizedStem),np.array(normalizedFourWay)]
axes[2].plot([0,1,2,3],[np.median(normalizedStem)]*4,color=(0.5,0.5,0.5),linestyle='dashed')
axes[2].plot([0,1,2,3],[np.median(normalizedFourWay)]*4,color=(0.5,0.5,0.5),linestyle='dashed')

bp=axes[2].boxplot(boxPlotData,showfliers=False)
...
dataAlreadySeen={}
baseValueX=1
xLinearGradient=0.05
multiplicativeFactor={True:1, False:-1}
for counter, eachEl in enumerate(normalizedStem):
    if eachEl in dataAlreadySeen:
        xToPlot=baseValueX+multiplicativeFactor[dataAlreadySeen[eachEl]%2==0]*xLinearGradient*((dataAlreadySeen[eachEl]/2)+1)
        dataAlreadySeen[eachEl]+=1
    else:
        xToPlot=baseValueX
        dataAlreadySeen[eachEl]=0
    axes[2].scatter(xToPlot,eachEl,c=(np.array(colorPaletteDictionary[namesSeq[counter]])/255.0).reshape((1,3)),edgecolors='none',s=15)

dataAlreadySeen={}
baseValueX=2    
for counter, eachEl in enumerate(normalizedFourWay):
    if eachEl in dataAlreadySeen:
        xToPlot=baseValueX+multiplicativeFactor[dataAlreadySeen[eachEl]%2==0]*xLinearGradient*((dataAlreadySeen[eachEl]/2)+1)
        dataAlreadySeen[eachEl]+=1
    else:
...
binWidth=5.0
bins=np.arange(binWidth/2.0,101.0,binWidth)
...
for eachEl in lengthRNAInTwoWay:
    for counter, eachEl1 in enumerate(bins):
        if eachEl>=lowerBins[counter] and eachEl<=upperBins[counter]:
            density1[counter]+=1
            break

for eachEl in lengthRNAInFourWay:
    for counter, eachEl1 in enumerate(bins):
        if eachEl>=lowerBins[counter] and eachEl<=upperBins[counter]:
            density2[counter]+=1
            break

axes[3].bar(bins,density1,width=binWidth,align='center',color=(0.5,0.5,0.5))
axes[4].bar(bins,density2,width=binWidth,align='center',color=(0.5,0.5,0.5))
...
plt.savefig('2dPlots_Final_ver4_20190503.pdf',dpi=300)
