...
def returnElementNotation(sequence):
    with open('dummy_structureCalc2way4wayStoreData_v2_20190416.txt','w') as f:
        processCall=subprocess.Popen(['RNAfold','--noPS','--noGU'],stdout=f,stdin=subprocess.PIPE)
        processCall.communicate(input=sequence)
    dotBracket=returnDotBracket('dummy_structureCalc2way4wayStoreData_v2_20190416.txt')
...
def writeNewClustal(str1,str2,consStr):
    with open('dummyClustal_structureCalc2way4wayStoreData_v2_20190416.aln','w') as wf:
...
with open('structureCalc2way4wayStoreData_v2_20190416_allLists.pckl','wb') as f:
    cPickle.dump([namesSeq,actualSeq,twoWayRepeatCoord,fourWayRepeatCoord,blastCoord],f,protocol=cPickle.HIGHEST_PROTOCOL)
