...
toPlotComb=['ATAT','TATA','CGCG','GCGC']
...
newObjDictNames=[
    ...
    'AF_SOL_722_5_trimmomaticP_thresholdTestDict_1_seqErrorCorrect_1_newObjDict.pckl',
    'AF_SOL_713_Sample6_trimmomatic.fast_thresholdTestDict_1_seqErrorCorrect_1_newObjDict.pckl'
]
fourWayDiversityCoord=[[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,2,3,5],[0,1,3,4],[0,1,3,4]]
...
for counter, eachNewObjDictName in enumerate(newObjDictNames):
    ...
    relevantCoord=fourWayDiversityCoord[counter]
    relevantDiversityPositions=numberDiversityPositions[counter]
    for eachRef in newObjDict:
        if eachRef[0]=='G': #use 4-way combinations as defined on G strand but sum for both G and C strands
            takeRevComp=0
        elif eachRef[0]=='C':
            takeRevComp=1
            ...
            if isFullLength(newObj):
                ...
                if len(divString)==relevantDiversityPositions and returnNumberMutations(newObj.alignmentInfo[1])<numberMutationLimit:
                    ...
                    for eachEl in relevantCoord:
                        fourWayComb+=divString[eachEl]
                    if takeRevComp==0:
                        dataDict[fourWayComb]+=newObj.count[pcrDuplicates]
                    elif takeRevComp==1:
                        dataDict[revComp(fourWayComb)]+=newObj.count[pcrDuplicates]
...
plt.savefig('barPlot_20180402_fourWay_ver2.pdf',dpi=300)
#plt.show()
